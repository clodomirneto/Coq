...
Inductive Gcf : list FProp -> list FProp -> Prop :=
...
Fixpoint AtomicF (P:Atomic) : FProp := match P with
| AVar P => #P
| ABot => ⊥
end.
...
destruct(VarSeq_dec v v0);(left; f_equal; assumption) || (right; intro HH; injection HH; contradiction).
Qed.
...
intros; induction Γ.
exists []; reflexivity.
destruct a; try (apply plus_is_O in H as (?&_); simpl in H; discriminate); unfold sizel in H;simpl in H;
destruct (IHΓ H);[exists (AVar v :: x)|exists (ABot :: x)]; simpl; f_equal; assumption.
Qed.
...
rewrite Heqv;simpl. destruct (in_dec Atomic_eqdec (AVar v0) x);[exact I|contradiction].
contradiction.
apply in_map_iff in H1 as (?&?&?);subst x1. destruct x2.
constructor 1 with v0.
change #v0 with (AtomicF (AVar v0));apply in_map.
simpl in H2;rewrite Heqv in H2. destruct (in_dec Atomic_eqdec (AVar v0));[assumption|contradiction].
change #v0 with (AtomicF (AVar v0)). apply in_map. assumption.
contradiction.
