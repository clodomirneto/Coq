...
Inductive G : list FProp -> list FProp -> Prop :=
...
apply ImpI.
apply BotC.
apply ImpE with (A ∧ B);[is_ass|apply AndI; apply BotC;(apply ImpE with (¬A ∨ ¬B);[is_ass|])].
apply OrI1; is_ass.
apply OrI2; is_ass.
Qed.
...
[apply IHNc..|apply IHNc2|try apply IHNc3];(split;[is_ass|eapply Nc_list_weakening;[|eassumption];in_solve]).
Qed.
...
try apply in_split_app in Heql as [(?&?&?)|(?&?&?)]; subst; try (temp2 IHG IHG1 IHG2;fail);[is_ass|constructor 2;in_solve|
